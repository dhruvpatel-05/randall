...
.PHONY: default clean assignment submission-tarball repository-tarball \
        check format valgrind sanitizers
...
# Runs the Undefined Behavior (UBSAN) and Address (ASAN) sanitizers. 
# GCC doesn’t support these on the SEASNet servers so recommend clang.
sanitizers:
